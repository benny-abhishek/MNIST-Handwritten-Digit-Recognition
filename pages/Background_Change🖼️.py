...
def app():
    ...
    st.title(":red[MNIST Handwritten Digit ( Background Change Transformation)]")
    ...
    def modify_background(image):
    #(255-white)
        new_background = np.full_like(image, 255)  # Replace with your desired background
    
        modified_image = np.where(image == 0, new_background, image)
    
        return modified_image
    
    X_train_modified = np.array([modify_background(img) for img in X_train])
    y_train_modified = np.array([modify_background(img) for img in y_train])
    
    st.subheader("Sample for Background Changed Dataset")
    fig=plt.figure(figsize=(3, 0.3))
    for i in range(8):
        plt.subplot(1, 8, i+1)
        plt.imshow(X_train_modified[i].reshape(28, 28), cmap='gray')
        plt.axis('off')
    st.pyplot(fig)

    X_train_augmented = X_train_modified.reshape(X_train_modified.shape[0], 28*28) / 255.0
    y_train_augmented=y_train_modified
